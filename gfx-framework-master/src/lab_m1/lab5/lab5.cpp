...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-2, 4, 4));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-2, 5, 8));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_W)) {
            ...
            camera->TranslateForward(mouseSenzitivity * deltaTime);
        }
        ...
        if (window->KeyHold(GLFW_KEY_X) && !isPersp) {
            bottom -= deltaTime;
            top += deltaTime;
            projectionMatrix = glm::ortho(-5.0f, 5.0f, bottom, top, -5.0f, 5.0f);
        }

        if (window->KeyHold(GLFW_KEY_Z) && !isPersp) {
            bottom += 3* deltaTime;
            top -= 3 * deltaTime;
            projectionMatrix = glm::ortho(-5.0f, 5.0f, bottom, top, -5.0f, 5.0f);
        }
