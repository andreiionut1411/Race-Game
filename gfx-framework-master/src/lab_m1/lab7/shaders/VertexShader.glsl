...
void main()
{
    ...
    float specular_light = material_ks * 1 * recv_light * pow (max (dot (v_normal, H), 0), material_shininess);
    ...
    float distance = distance (world_pos, light_position);
    float light = ambient_light + (1 / (distance * distance)) * (diffuse_light + specular_light);
